...
def main():
    ...
    for row in data:
        ...
        for col in NUMERIC_COLS:
            ...
            # If stenosis is 0% and column is LE/TE, set to NaN
        ...
        for nz in non_zero_micro_runs:
            ...
            for z in zero_micro_runs:
                ...
                for col in NUMERIC_COLS:
                    ...
                    # val_z = safe_float(z.get(col, None))
                    #
                    # if is_zero(val_nz) and is_zero(val_z):
                    #     ratio_dict[col] = 1.0
                    ...
                    if (val_nz is None or is_zero(val_nz)) and (val_z is None or is_zero(val_z)):
                        ...
                        ratio_dict[col] = "∞"  # division by zero case
                    elif val_nz is not None and val_z and not is_zero(val_z):
                        ratio_dict[col] = val_nz / val_z
                    else:
                        ratio_dict[col] = None
    ...
    numeric_data = df_ratios[NUMERIC_COLS].applymap(
        lambda x: np.nan if isinstance(x, str) or x is None else x
    )
    ...
    cbar_min = 0.5
    cbar_max = 1.5
    cbar_mid = cbar_min/2 + cbar_max/2
    cax = ax.imshow(ratio_matrix,
                    aspect="auto", vmin=cbar_min, vmax=cbar_max, cmap=cmap)
    ...
    ax.set_yticklabels(sten_labels, fontsize=14, fontweight='bold')
    ...
    for i in range(ratio_matrix.shape[0]):
        for j in range(ratio_matrix.shape[1]):
            ...
            val = df_ratios.iloc[i][NUMERIC_COLS[j]]
            raw_nz = safe_float(df_ratios.at[i, "R_micro_nonzero"])
            stenosis = df_ratios.at[i, "Stenosis"]
            colname = NUMERIC_COLS[j]

            if val == "0/0":
                ax.text(j, i, "0 / 0", ha="center", va="center",
                        fontsize=12, fontweight="bold", color="black")
            elif val == "∞":
                ax.text(j, i, "∞", ha="center", va="center",
                        fontsize=14, fontweight="bold", color="black")
            elif pd.isna(val) and (stenosis < 0.01) and ("LE" in colname or "TE" in colname):
    ...
    cbar = fig.colorbar(cax, ticks=[cbar_min, cbar_mid, cbar_max])
    cbar.set_label("Ratio (New / Old)", fontsize=18, labelpad=5)  # label + spacing
    cbar.ax.tick_params(labelsize=18)  # increase tick font size

    ax.set_xticks(range(len(NUMERIC_COLS)))
    # ax.set_xticklabels(NUMERIC_COLS, rotation=45)
    display_names = [DISPLAY_LABELS.get(col, col) for col in NUMERIC_COLS]
    ax.set_xticklabels(display_names, rotation=45, fontsize = 12, fontweight = 'bold')
