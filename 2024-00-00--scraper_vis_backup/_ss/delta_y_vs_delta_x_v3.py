...
def build_steps_df_for_cutoff(
    ...
    *,
    ...
    """
    Build a per-step dataframe for cutoff detection.

    Returns a dataframe with:
      - series  : tuple key identifying each series
      - step    : step index within that series
      - hmr_mid : geometric midpoint of x over the step (name retained for compatibility;
                 it's the midpoint of x_col even if x_col != 'HMR')
      - sens    : per-step sensitivity (log–log elasticity by default)

    include_no_stenosis:
      - True  -> keep stenosis group 0.0
      - False -> drop stenosis group 0.0
    """
    dfp = data.copy()
        ...
        for i in range(len(g) - 1):
            ...
            if use_loglog:
                ...
                if dlogx == 0:
                    continue
                sens = np.log(y1 / y0) / dlogx
                x_mid = np.sqrt(x0 * x1)  # geometric midpoint on log scale
            else:
...
def find_cutoff_piecewise_constant_ruptures(
    ...
    *,
    ...
    """
    B1 cutoff via ruptures (one breakpoint) + within-series permutation test.

    - Sorts by hmr_mid (x-axis)
    - Uses ruptures Binseg with L2 cost (piecewise-constant mean shift)
    - Enforces minimum segment length via min_size=min_per_side
    - Permutation test: shuffles sens WITHIN each series to preserve dependence

    Returns dict similar to your previous function.
    """
    rng = np.random.default_rng(seed)
    ...
    df = steps_df.dropna(subset=["hmr_mid", "sens", "series"]).copy()
    if len(df) < 2 * min_per_side + 1:
        raise RuntimeError(
            f"Not enough step observations ({len(df)}) for min_per_side={min_per_side}."
        )

    # Sort once by HMR midpoint (this defines the 1D order for ruptures)
    df = df.sort_values("hmr_mid").reset_index(drop=True)

    x = df["hmr_mid"].to_numpy(float)
    y = df["sens"].to_numpy(float)

    def _segment_once(y_vec):
        # ruptures expects shape (n_samples, n_features)
        signal = y_vec.reshape(-1, 1)

        algo = rpt.Binseg(model=model, min_size=int(min_per_side), jump=1)
        algo = algo.fit(signal)

        bkpts = algo.predict(n_bkps=1)  # list includes final index n
        # For n_bkps=1, bkpts usually looks like [t, n]
        t = bkpts[0]

        # Guardrails
        n = len(y_vec)
        if not (min_per_side <= t <= n - min_per_side):
            # If it violates, something is off (or min_size not respected due to edge cases)
            raise RuntimeError(
                f"Ruptures returned invalid breakpoint t={t} for n={n}, min_per_side={min_per_side}."
            )

        mu_lo = y_vec[:t].mean()
        mu_hi = y_vec[t:].mean()
        sse = ((y_vec[:t] - mu_lo) ** 2).sum() + ((y_vec[t:] - mu_hi) ** 2).sum()

        # Define cutoff on x-axis between x[t-1] and x[t]
        cutoff = float(np.sqrt(x[t - 1] * x[t]))  # geometric mean boundary

        return t, cutoff, float(mu_lo), float(mu_hi), float(sse)

    # Observed segmentation
    t_obs, cutoff_obs, mu_lo_obs, mu_hi_obs, sse_obs = _segment_once(y)

    # Null SSE: single mean
    mu_all = y.mean()
    sse_null = float(((y - mu_all) ** 2).sum())
    improvement_obs = sse_null - sse_obs  # larger => stronger evidence of cutoff

    # Permutation test: shuffle sens within series, keep x-order fixed
    improvements = np.empty(int(n_perm), dtype=float)

    # Pre-store indices per series in the sorted dataframe
    series_to_idx = df.groupby("series").indices

    for k in range(int(n_perm)):
        y_perm = y.copy()
        for s, idx in series_to_idx.items():
...
# Build steps_df for cutoff detection (HMR cutoff on FRR sensitivity)
steps_df = build_steps_df_for_cutoff(
    ...
    location_col="Location",
    location_filter=location_filter,
    include_no_stenosis=False,   # <-- your requested flag
    use_loglog=True,
)
...
result = find_cutoff_piecewise_constant_ruptures(
    steps_df,
    min_per_side=6,
    n_perm=5000,
    seed=1,
