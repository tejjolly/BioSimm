...
plt.rcParams.update({
    'figure.dpi': 300      # Higher DPI for crisp text
})
...
# ---------------------------------------------------
# PLOT 5) CFR vs FFR, colored by CFR/FFR, Garcia in gray circle
# ---------------------------------------------------
...
boundaries_5 = np.linspace(1, 3, 3)
cmap5 = plt.get_cmap('RdYlGn')
norm5 = colors.BoundaryNorm(boundaries_5, ncolors=256, clip=True)

sc5 = ax.scatter(
    df_p5['P_d/P_a'], df_p5['CFR'],
    c=df_p5['CFR/FFR'],
    cmap=cmap5, norm=norm5,
    edgecolor='k', alpha=1.0, s=60, label = 'Our Data'
)
cbar5 = fig.colorbar(sc5, ax=ax, ticks=boundaries_5)
cbar5.set_label('CFR/FFR', fontsize=12)

ax.axhline(y=2.0, color='gray', linestyle='--', linewidth=0.8)
ax.axvline(x=0.8, color='gray', linestyle='--', linewidth=0.8)
ax.set_xlabel('FFR')
ax.set_ylabel('CFR')
ax.set_title('CFR vs FFR, Colored by CFR/FFR')

if not df_g5.empty:
    ax.scatter(
        df_g5['P_d/P_a'], df_g5['CFR'],
        alpha=0.5, c=df_g5['CFR/FFR'],
        cmap=cmap5, norm=norm5,
...
# ---------------------------------------------------
# PLOT 6) CFR vs FFR, colored by HMR, Garcia in gray circle
# ---------------------------------------------------
fig, ax = plt.subplots(figsize=(6, 4))
...
boundaries_5 = np.linspace(0.9, 3, 4)
...
cbar5 = fig.colorbar(sc5, ax=ax, ticks=boundaries_5)
cbar5.set_label('HMR', fontsize=12)

ax.axhline(y=2.0, color='gray', linestyle='--', linewidth=0.8)
ax.axvline(x=0.8, color='gray', linestyle='--', linewidth=0.8)
ax.set_xlabel('FFR')
ax.set_ylabel('CFR')
ax.set_title('CFR vs FFR, Colored by HMR')

if not df_g5.empty:
    ax.scatter(
        df_g5['P_d/P_a'], df_g5['CFR'],
        alpha=0.5, c=df_g5['HMR'],
        cmap=cmap5, norm=norm5,
        edgecolors='gray', s=g_size, label=g_label
    )
    ax.legend(loc='best')
...
# ---------------------------------------------------
# PLOT 7) CFR vs FFR, colored by Stenosis Percentage
# ---------------------------------------------------
fig, ax = plt.subplots(figsize=(6, 4))
...
df_p5 = df_mine[
    df_mine['P_d/P_a'].notna() &
    df_mine['CFR'].notna() &
    df_mine['Stenosis Percentage'].notna()
]
df_g5 = df_garcia[
    df_garcia['P_d/P_a'].notna() &
    df_garcia['CFR'].notna()
...
boundaries_5 = np.linspace(0, 0.7, 6+1)
cmap5 = plt.get_cmap('RdYlGn_r')
norm5 = colors.BoundaryNorm(boundaries_5, ncolors=256, clip=True)

sc5 = ax.scatter(
    df_p5['P_d/P_a'], df_p5['CFR'],
    c=df_p5['Stenosis Percentage'],
    cmap=cmap5, norm=norm5,
    edgecolor='k', alpha=1.0, s=60, label = 'Our Data'
)
cbar5 = fig.colorbar(sc5, ax=ax, ticks=boundaries_5)
cbar5.set_label('Stenosis Percentage', fontsize=12)

ax.axhline(y=2.0, color='gray', linestyle='--', linewidth=0.8)
ax.axvline(x=0.8, color='gray', linestyle='--', linewidth=0.8)
ax.set_xlabel('FFR')
ax.set_ylabel('CFR')
ax.set_title('CFR vs FFR, Colored by Stenosis Percentage')

if not df_g5.empty:
    ax.scatter(
        df_g5['P_d/P_a'], df_g5['CFR'],
        alpha=0.5, c=df_g5['Stenosis Percentage'],
        cmap=cmap5, norm=norm5,
        edgecolors='gray', s=g_size, label=g_label
    )
    ax.legend(loc='best')
...
############################################################################
def plot_four_subplots(set_limits=False):
    ...
    if not df_g3.empty:
        ax3.scatter(
            ...
            alpha=g_alpha, c=df_g3['P_Loss_Coeff'], cmap=cmap3, edgecolors='none', s=g_size, label=g_label
        )
        ax3.legend(loc='best')
    ...
    if not df_g4.empty:
        ax4.scatter(
            ...
            alpha=g_alpha, c=df_g4['BMR/HMR'],
            cmap=cmap4, norm=norm4,
            edgecolors='none', s=g_size, label=g_label
        )
        ax4.legend(loc='best')

    # ── Optional: enforce fixed limits ─────────────────────────────────────────────
    if set_limits:
        ax1.set_xlim(0.0013227518999999743, 14.3672839461)
        ax1.set_ylim(-0.427356418,         4.21366764)

        ax2.set_xlim(-0.427356418,         4.21366764)
        ax2.set_ylim(0.21284793550748599,  1.0301933543427941)

        ax3.set_xlim(0.0013227518999999743, 14.3672839461)
        ax3.set_ylim(0.21284793550748599,  1.0301933543427941)

        ax4.set_xlim(0.21300429216916733,   1.0269098644474863)
        ax4.set_ylim(0.6706097561,          5.717195121900001)

    plt.tight_layout()

    # (Optional) echo the limits so you can verify they took effect
    for i, ax in enumerate([ax1, ax2, ax3, ax4], 1):
        print(f"Subplot {i}: xlim = {ax.get_xlim()}, ylim = {ax.get_ylim()}")

    plt.show()

############################################################################
#  If you want a single 2x2 figure with these four plots, just call:
# plot_four_subplots(set_limits=True)
############################################################################
