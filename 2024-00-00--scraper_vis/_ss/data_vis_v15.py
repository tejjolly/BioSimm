...
plot15 = True # CFR vs FFR, col HMR
plot2  = False # HMR vs HSR, col FFR
plot3  = False # FFR vs HMR, col HSR
...
if plot1:
    ...
    df_filtered_cfr = df[(df['CFR'].notna()) & (df['P_d/P_a'].notna()) & (df['source'] == 'mine')]
...
if plot11:
    # PLOT 1) WSS_LE_Area_min vs HMR, colored by HSR
    # df['WSS_LE_Area_min'] = np.log10(df['WSS_LE_Area_min'])
    df_filtered = df[
        (df['Condition'] == 'Hyperemic') &
        df['HMR'].notna() &
        df['HSR'].notna() &
        df['WSS_LE_Area_min'].notna() &
        (df['WSS_LE_Area_min'] != 0)
    # (df['R_micro'] == 0) &
        # (df['Location'] == 'LAD')
        ]
    boundaries_third = np.linspace(0.1, 1.1, 6)
    make_smart_scatter(
        data=df_filtered,
        x_col='HMR', y_col='WSS_LE_Area_min', color_col='HSR',
        x_label='HMR [mmHg/cm/s]', y_label='WSS_LE_Area_min',
        title='WSS_LE_Area_min vs HMR, Colored by HSR',
        cmap_name='RdYlGn_r',
        custom_boundaries=boundaries_third,
        color_label='HSR [mmHg/cm/s]',
        # add_threshold=[{'axis': 'y', 'value': 0.8}],
