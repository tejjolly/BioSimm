...
# def stenosis_group(val, decimal_places=2, tolerance=0.03):
#     """
...
### THIS VERSION IS FOR SNAPPING
def stenosis_group(val, anchors=(0.0, 0.45, 0.60), tol=0.03):
    """
    Snap val to nearest anchor if within ±tol; else NaN.
    """
    if pd.isna(val):
        return np.nan
    nearest = min(anchors, key=lambda a: abs(val - a))
    return nearest if abs(val - nearest) <= tol else np.nan
...
if plot25:
    # PLOT 2) HSR vs HMR, colored by flowrate
    df_filtered = df[
        (df['Condition'] == 'Hyperemic') &
        df['HMR'].notna() &
        df['HSR'].notna() &
...
if plot18:
    ...
    make_smart_scatter(
        ...
        x_label='HMR [mmHg/cm/s]', y_label='Distal flowrate [cm3/s]',
...
if plot19:
    ...
    make_smart_scatter(
        ...
        x_label='HSR [mmHg/cm/s]', y_label='Distal flowrate [cm3/s]',
