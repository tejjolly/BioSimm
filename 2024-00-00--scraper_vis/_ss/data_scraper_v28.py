...
"""
"""
NOTES: Adding temporary Q_distal based on eyeballed area distal to LAD=0.095 cm2
"""
import os
import csv
import re
...
### Pressure plot stuff
out_dir  = os.path.join(base_dir, 'Post_Processing/an-2024-00-00--scraper_vis/images')
...
for run in data:
    if run['Condition'] == 'Hyperemic' and run['source'] == 'mine':
        ...
        if matching_ifr:
            # a) Compute CFR using distal velocity: CFR = v_distal(hyperemic) / v_distal(baseline)
            try:
...
# ----------------------------------------------------------------------------
# WRITE SELECTED ROWS TO data_manuscript.csv
# ----------------------------------------------------------------------------
manuscript_pairs = [
    ("Non-hyperemic", "1"),
    ("Hyperemic", "1"),
...
missing = []  # keep track of pairs we couldn't find

with open(manuscript_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(header)  # reuse your existing header

    # Write rows in the exact order (and with duplicates) as provided in manuscript_pairs
    for cond, geom in manuscript_pairs:
        ...
        # Duplicate entries in manuscript_pairs will write duplicate rows on purpose.
        run = index[key][0]
...
if split_plots:
    ...
    make_combo_plot(cond_filter="Hyperemic", filename_suffix="_hyperemic", add_legend=False, flip_sign=True)
else:
