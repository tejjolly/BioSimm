...
def plot_ffr_cfr_discordance_stacked(
    ...
    *,
    ...
    for col in [ffr_col, cfr_col, width_col]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df.dropna(subset=[ffr_col, cfr_col, width_col])

    # ---- Lesion type ----
    df["lesion_type"] = np.where(df[width_col] < focal_threshold, "focal lesion", "diffuse lesion")
...
fig, ax, ct, pval, testname = plot_ffr_cfr_discordance_stacked(df, focal_threshold=0.75, ffr_thresh=0.80, cfr_thresh=2.0,
                                                               use_test="chi2", save_path="ffr_cfr_discordance.svg")
print(ct)
print(testname, pval)