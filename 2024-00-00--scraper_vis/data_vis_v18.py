...
plot25  = True  # HSR vs HMR, col Average Flow
...
HSR_boundaries = np.linspace(0.0, 1.6, 5)
...
def _scatter_kwargs(df, *, alpha, edgecolor, linewidths=None, marker='o', zorder=3):
    kw = dict(
        x=df[x_col],
        y=df[y_col],
        alpha=alpha,
...
def make_smart_scatter(
    ...
    # Color scaling bounds should be computed from whatever colored points exist.
...
if plot25:
    ...
    make_smart_scatter(
        data=df_filtered,
        x_col='HMR', y_col='HSR', color_col='Q_distal',
        x_label='HMR [mmHg/cm/s]', y_label='HSR [mmHg/cm/s]',
