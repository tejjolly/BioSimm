...
plot1   = True # CFR vs FFR, col BMR/HMR
...
plot13  = False # Area Low Bif WSS -x-x-x-x            #Fig 6
plot14  = False  # Area High LE WSS x-x-x-x            #Fig 6
plot15  = False  # Area High TE WSS x-x-x-x            #Fig 6
plot16  = False # Area High Bif WSS x-x-x-x-x            #Fig 6
...
plot22  = False # P_Loss_Coeff vs HMR, col FFR           # Fig8
plot23  = False # FFR vs P_Loss_Coeff, col HMR           # Fig8
plot24  = False # FFR vs HMR, col P_Loss_Coeff           # Fig8
plot26  = False # FFR vs Q_distal, col HSR

fig7_force_equal_size = True
...
selected_sources = [source for source, enabled in SOURCE_SWITCHES.items() if enabled]
selected_external_sources = [source for source in selected_sources if source != 'mine']
selected_external_data_source = (
    selected_external_sources if selected_external_sources else None
)

if not selected_sources:
    raise ValueError("Set at least one of mine_data, arnav_data, or garcia_data to True.")

if manuscript_mode == poster_mode:
    raise ValueError("Set exactly one of manuscript_mode or poster_mode to True.")

if manuscript_mode:
    FIG4SIZE = (6.5, 4) # FFR and CFR vs. HMR col HSR
    FIG4CMAPFLOOR = 0.25
    FIG4CMAPCEIL = 0.9
    
    FIG5SIZE = (7, 5)
    FIG6SIZE = (8, 5) # WSS Plots
    FIG7SIZE = (8.5, 5) # External data overlays
    FIG8SIZE = (7.5, 5) # CFR vs FFR col HMR
    ...
    FIG4SIZE = (7, 4) # FFR and CFR vs. HMR col HSR
    FIG4CMAPFLOOR = 0.15
    FIG4CMAPCEIL = 1.0
    
    FIG5SIZE = (7, 5)
    FIG6SIZE = (6.5, 4) # WSS Plots
    # FIG7SIZE = (8.5, 5) # External data overlays
    scale = 0.6
    FIG7SIZE = (8 * scale, 5 * scale) # External data overlays
    FIG8SIZE = (7.25, 4) # CFR vs FFR col HMR
...
FIG7_MAIN_AXES_RECT = (0.15, 0.15, 0.65, 0.74)
FIG7_COLORBAR_RECT = (0.825, 0.15, 0.025, 0.74)
FIG7_THRESHOLD_LINE_THICKNESS = 3
...
if 'source' not in df.columns:
    raise ValueError(f"Expected a 'source' column in {data_file}.")
...
def make_smart_scatter(
        ...
        force_equal_plot_area=False,
):
    fig = plt.figure(figsize=figsize)
    if force_equal_plot_area:
        ax = fig.add_axes(FIG7_MAIN_AXES_RECT)
    else:
    ...
    if has_color_dimension and first_scatter is not None:
        if force_equal_plot_area:
            cax = fig.add_axes(FIG7_COLORBAR_RECT)
            cbar = fig.colorbar(first_scatter, cax=cax, ticks=custom_boundaries)
        else:
            cbar = fig.colorbar(first_scatter, ax=ax, ticks=custom_boundaries)
        if color_label:
        ...
        if force_equal_plot_area:
            cax = fig.add_axes(FIG7_COLORBAR_RECT)
            cbar = fig.colorbar(mono_sm, cax=cax, ticks=hmr_ticks)
        else:
            cbar = fig.colorbar(mono_sm, ax=ax, ticks=hmr_ticks)
    ...
    if add_threshold:
        for tdict in add_threshold:
            ...
            if axis_type == 'y':
                ax.axhline(y=value, color=c, linestyle=style, linewidth=w)
            else:
                ax.axvline(x=value, color=c, linestyle=style, linewidth=w)
    ...
    if savefig:
        ...
        fig.savefig(f'{dir}/{fname}.png', dpi=dpi, transparent=True, **savefig_kwargs)
        fig.savefig(f'{dir}/{fname}.svg', transparent=True, **savefig_kwargs)
...
if plot1:
    ...
    make_smart_scatter(
        ...
        color_label='HMR',
        add_threshold=[
            {'axis': 'y', 'value': 2.0, 'style': '--', 'color': 'gray', 'width': FIG7_THRESHOLD_LINE_THICKNESS},
            {'axis': 'x', 'value': 0.8, 'style': '--', 'color': 'gray', 'width': FIG7_THRESHOLD_LINE_THICKNESS}
        ],
        connect_stenosis_groups=False,
        show_singletons=True,
        external_data_source=selected_external_data_source,
        external_style=EXTERNAL_STYLE,
        savefig=True,
        figsize=FIG7SIZE,
        force_equal_plot_area=False,
...
if plot1_5:
    ...
    make_smart_scatter(
        ...
        savefig=True,
        labels=False,
        alpha_scatter=FIG8ALPHA,
        location_filter=location_filter,
        figsize=FIG8SIZE,
        cmap_ceil=FIG8CMAPCEIL,
        cmap_floor=FIG8CMAPFLOOR
    )
...
if plot36:
    ...
    make_smart_scatter(
        ...
        cmap_name='Reds',
        cmap_floor=FIG4CMAPFLOOR,
        cmap_ceil=FIG4CMAPCEIL,
...
if plot22:
    ...
    make_smart_scatter(
        ...
        savefig=True,
        figsize=FIG7SIZE,
        force_equal_plot_area=fig7_force_equal_size
...
if plot23:
    ...
    make_smart_scatter(
        ...
        color_label='HMR [mmHg/cm/s]',
        add_threshold=[
            {'axis': 'y', 'value': 0.8, 'style': '--', 'color': 'gray', 'width': FIG7_THRESHOLD_LINE_THICKNESS},
            # {'axis': 'x', 'value': 0.8, 'style': '--', 'color': 'gray', 'width': 0.8}
        ],
        connect_stenosis_groups=False,
        show_singletons=True,
