...
"""
End-to-end script:
1) Load/clean manuscript CSV
2) Build step midpoints + stepwise log–log elasticities (sens)
3) Estimate 1 change-point (B1) in mean sensitivity using ruptures (Binseg, L2)
4) Compute a within-series permutation p-value
5) Plot y vs x with:
   - original points (colored by color_col)
   - connected series lines (by length / vessel)
   - red circle midpoints
   - red dashed vertical breakpoint line
...
Y_COL_ANALYSIS = "FFR" # sensitivity computed for this variable (e.g., "P_d/P_a" for FFR, "CFR" for CFR)
Y_COL_PLOT = Y_COL_ANALYSIS     # plotted on y-axis (often same as Y_COL_ANALYSIS)
COLOR_COL = "HSR"      # point coloring for plot
