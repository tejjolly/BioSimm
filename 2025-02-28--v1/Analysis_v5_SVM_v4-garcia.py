...
required_cols = ['P_d/P_a','P_Loss_Coeff','CFR','CFR/FFR','HMR','BMR/HMR','discord','source']
...
idx_train_full, idx_test = train_test_split(
    all_indices, test_size=test_size, stratify=y_dummy, random_state=39
)
...
for (target_col, target_name) in targets:
    for (xcol, ycol) in predictor_pairs:
        ...
        ax.set_xlabel(xcol)
        ax.set_ylabel(ycol)
        ax.set_title(f"{target_name}\nAcc: {test_acc:.2f} (Test size: {test_size*100:.0f}%)")
