...
output_cols = ['FFR', 'CFR']#, 'CFR/FFR']  # Multi‐output: 2 targets
...
###############################################################################
# 3) 5-FOLD CV ON THE TRAINING SET (Mean ± Std R² over both outputs)
###############################################################################
# We'll build a pipeline that scales X -> linear reg for multi-output Y.
# Then compute cross_val_score with cv=KFold(5).
...
cv_scores = cross_val_score(
    ...
    X_train,
    Y_train,
    scoring='r2',  # default: average R^2 across outputs
    cv=kf
)
...
print(f"5-fold CV R^2 (avg across FFR & CFR): mean={cv_mean:.3f}, std={cv_std:.3f}")
