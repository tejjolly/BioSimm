...
FIGSIZE_PHASE2 = (3, 2)      # for phase-2 concentration plots
FIGSIZE_PHASE3 = (3, 2)      # for TAG vs HMR plots (combined + individual)
...
def phase3_calc_tafe_and_plot_hmr():
    ...
    for geom, dye in series_keys:
        ...
        dye_name = DYE_LABELS.get(dye, dye)
        if PLOT_TITLES:
            ax_s.set_title(f"{geom_name}, Dye-{dye_name}")
        ax_s.set_ylim([-0.35,0])
        plt.tight_layout()
