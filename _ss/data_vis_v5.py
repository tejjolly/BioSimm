...
for name, group in grouped_hmr:
    ...
    if len(unique_r_values) > 1:
        ...
        plt.text(mean_hmr, mean_hsr, f'S:{stenosis_formatted}%, L:{length_val}',
                 fontsize=10, color='gray', ha='center', va='center')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
...
# Normalize the 'HSR' values for color mapping
norm_third = colors.Normalize(vmin=df_filtered_third['HSR'].min(), vmax=df_filtered_third['HSR'].max())
...
# -----------------------------
# Fourth Plot: FFR vs HSR, Colored by HSR
# -----------------------------
...
for name, group in grouped_fourth:
    ...
    if len(unique_r_values) > 1:
        ...
        mean_hsr = group['HSR'].mean()
        mean_ffr = group['P_d/P_a'].mean()
        plt.text(mean_hsr, mean_ffr, f'S:{stenosis_formatted}%, L:{length_val}',
                 fontsize=10, color='gray', ha='center', va='center')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
