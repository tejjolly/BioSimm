...
summary_file = '/Users/tejjolly/Documents/BioSimm/Simulations/Post_Processing/summary.csv'          # All runs
df = pd.read_csv(summary_file)

# Convert columns to numeric, coercing errors to NaN
cols_to_num = [
    'CFR', 'P_d/P_a', 'BMR/HMR', 'Rtotal_cor Value',
...
# -----------------------------
# (Assuming your previous data reading and preprocessing code has run)
# -----------------------------

# -----------------------------
# 6) 3-D PLOT WITH REGRESSION PLANE: FFR (z) vs HSR (y) and HMR (x)
# -----------------------------
# Use an interactive backend if you want to rotate the plot (e.g., in Spyder run %matplotlib qt)
# Filter data: only include rows with hyperemic condition and non-NaN values for HMR, HSR, and FFR.
df_filtered_3d = df[
    (df['Condition'] == 'Hyperemic') &
    df['HMR'].notna() &
    df['HSR'].notna() &
...
# Extract independent variables (HMR and HSR) and the dependent variable (FFR)
X = df_filtered_3d[['HMR', 'HSR']]
y = df_filtered_3d['P_d/P_a']
...
print("  Intercept: {:.3f}".format(reg_model.intercept_))
print("  Coefficients (HMR, HSR):", reg_model.coef_)

# --- Create a grid over the range of HMR and HSR for the regression plane ---
hmr_min, hmr_max = X['HMR'].min(), X['HMR'].max()
hsr_min, hsr_max = X['HSR'].min(), X['HSR'].max()

# Create a grid with more points for a smoother surface
hmr_lin = np.linspace(hmr_min, hmr_max, 30)
hsr_lin = np.linspace(hsr_min, hsr_max, 30)
hmr_grid, hsr_grid = np.meshgrid(hmr_lin, hsr_lin)

# Flatten the grid arrays and prepare for prediction
grid_points = np.c_[hmr_grid.ravel(), hsr_grid.ravel()]
ffr_grid = reg_model.predict(grid_points).reshape(hmr_grid.shape)
...
scatter = ax.scatter(
    X['HMR'], X['HSR'], y,
    c=y, cmap='RdYlGn', s=50, edgecolor='k', label='Data points'
)

# Plot the regression plane (alpha controls transparency)
plane = ax.plot_surface(
    hmr_grid, hsr_grid, ffr_grid,
    alpha=0.5, color='r', rstride=1, cstride=1, edgecolor='none'
)

# Set axis labels and title
ax.set_xlabel('HMR [mmHg/cm/s]')
ax.set_ylabel('HSR [mmHg/cm/s]')
ax.set_zlabel('FFR (P_d/P_a)')
ax.set_title('3D Regression: FFR vs HMR and HSR')

# Optionally, adjust the view angle for a better initial presentation
ax.view_init(elev=30, azim=45)

# Add a colorbar for the scatter plot
plt.colorbar(scatter, pad=0.1, aspect=10, label='FFR (P_d/P_a)')
