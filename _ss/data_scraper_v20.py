        ...
        if os.path.isdir(item_path) and item.startswith('Geometry_'):
            ...
            if os.path.exists(flows_file):
                with open(flows_file, 'r') as f:
                    ...
                    header_line = next(reader, None)  # skip header
                    for row in reader:
                        if row:
                            flow_value = row[-1]
                            try:
        ...
        with open(garcia_file, 'r') as gf:
            ...
            for row in reader:
                ...
                garcia_dict = {
                    ...
                    'HMR': hmr_g,
                    'HSR': '',
                    # Keep p_loss_coeff from the CSV, do not overwrite if no v_distal
...
for run in data:
    ...
    try:
        ...
        p_loss_coeff_calc = (delta_p) / denom # *10 for Pa - dyne conversion
        p_loss_coeff_calc = np.log10(p_loss_coeff_calc)
        print(p_loss_coeff_calc)

        run['P_Loss_Coeff'] = p_loss_coeff_calc  # Overwrite any old value
    except (ValueError, ZeroDivisionError):
        # If anything is invalid or missing, leave P_Loss_Coeff as-is or blank
        run['P_Loss_Coeff'] = run['P_Loss_Coeff'] or ''

# ----------------------------------------------------------------------------
# 6) WRITE THE SUMMARY CSV FILE (with new columns)
# ----------------------------------------------------------------------------
header = [
    'Condition',
    'Geometry Number',
