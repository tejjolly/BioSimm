...
for run in data:
    if run['Condition'] == 'Non-hyperemic':
        ...
        try:
            ...
            max_flow_val = float(run['Max Flow']) if run['Max Flow'] != '' else None  ### NEW ###
...
for run in data:
    if run['Condition'] == 'Hyperemic':
        ...
        if matching_ifr:
            # a) Compute CFR = (Hyperemic MAX flow / Baseline (iFR) MAX flow)
