...
# -----------------------------
# Second Plot: HMS vs HMR, colored by FFR (P_d/P_a)
# -----------------------------

# Filter out rows where 'HMR', 'HMS', or 'P_d/P_a' are NaN
# Include only FFR runs (Condition == 'Hyperemic')
df_filtered_hmr = df[(df['Condition'] == 'Hyperemic') & df['HMR'].notna() & df['HMS'].notna() & df['P_d/P_a'].notna()]
...
scatter_hmr = plt.scatter(df_filtered_hmr['HMR'], df_filtered_hmr['HMS'],
...
for name, group in grouped_hmr:
    ...
    if len(unique_r_values) > 1:
        ...
        # Plot the line connecting the points
        plt.plot(group['HMR'], group['HMS'], linestyle='-', color='gray', alpha=0.5)
        # Optionally, add line labels if desired
        # Get the mean position to place the line label
        mean_hmr = group['HMR'].mean()
        mean_hms = group['HMS'].mean()
        plt.text(mean_hmr, mean_hms, f'S:{stenosis_formatted}%, L:{length_val}',
                 fontsize=10, color='gray', ha='center', va='center')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
            plt.text(row['HMR'], row['HMS'], f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')
...
plt.xlabel('HMR')
plt.ylabel('HMS')
plt.title('HMS vs HMR, Colored by FFR')
...
# -----------------------------
# Third Plot: FFR vs HMR, Colored by HMS
# -----------------------------

# Filter out rows where 'HMR', 'HMS', or 'P_d/P_a' are NaN
# Include only FFR runs (Condition == 'Hyperemic')
df_filtered_third = df[(df['Condition'] == 'Hyperemic') & df['HMR'].notna() & df['HMS'].notna() & df['P_d/P_a'].notna()]

# Define the colormap (from blue to red)
cmap_third = cm.get_cmap('RdYlGn')

# Normalize the 'HMS' values for color mapping
norm_third = colors.Normalize(vmin=df_filtered_third['HMS'].min(), vmax=df_filtered_third['HMS'].max())

# Create the figure for the third plot
plt.figure(figsize=(12, 8))

# Plot data points, colored by 'HMS'
scatter_third = plt.scatter(df_filtered_third['HMR'], df_filtered_third['P_d/P_a'],
                            c=df_filtered_third['HMS'], cmap=cmap_third, norm=norm_third, edgecolor='k', alpha=0.7)
# Group the data by 'Length' and 'Stenosis Percentage'
...
for name, group in grouped_third:
    ...
    if len(unique_r_values) > 1:
        ...
        mean_hmr = group['HMR'].mean()
        mean_ffr = group['P_d/P_a'].mean()
        plt.text(mean_hmr, mean_ffr, f'S:{stenosis_formatted}%, L:{length_val}',
                 fontsize=10, color='gray', ha='center', va='center')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
...
# Add colorbar for the 'HMS' values
cbar_third = plt.colorbar(scatter_third)
cbar_third.set_label('HMS')

# Add labels and title
plt.xlabel('HMR')
plt.ylabel('FFR')
plt.title('FFR vs HMR, Colored by HMS')
...
# -----------------------------
# Fourth Plot: FFR vs HMS, Colored by HMS
# -----------------------------

# The same filtered DataFrame can be used
...
# Normalize the 'HMS' values for color mapping
norm_fourth = colors.Normalize(vmin=df_filtered_third['HMR'].min(), vmax=df_filtered_third['HMR'].max())

# Plot data points, colored by 'HMS'
scatter_fourth = plt.scatter(df_filtered_fourth['HMS'], df_filtered_fourth['P_d/P_a'],
                             c=df_filtered_fourth['HMR'], cmap=cmap_third, norm=norm_fourth, edgecolor='k', alpha=0.7)
...
for name, group in grouped_fourth:
    ...
    if len(unique_r_values) > 1:
        ...
        group = group.sort_values(by='Rtotal_cor Value')
        # Plot the line connecting the points
        plt.plot(group['HMS'], group['P_d/P_a'], linestyle='-', color='gray', alpha=0.5)
        # Optionally, add line labels if desired
        mean_hms = group['HMS'].mean()
        mean_ffr = group['P_d/P_a'].mean()
        plt.text(mean_hms, mean_ffr, f'S:{stenosis_formatted}%, L:{length_val}',
                 fontsize=10, color='gray', ha='center', va='center')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
            plt.text(row['HMS'], row['P_d/P_a'], f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')

# Add colorbar for the 'HMS' values
cbar_fourth = plt.colorbar(scatter_fourth)
cbar_fourth.set_label('HMR')

# Add labels and title
plt.xlabel('HMS')
plt.ylabel('FFR')
plt.title('FFR vs HMS, Colored by HMS')
