...
garcia_file = os.path.join(base_dir, 'Post_Processing/garcia_data.csv')
garcia = True
...
if garcia:
    ...
    # (If any geometry #s are non-numeric, handle accordingly)
    largest_geom = 0
    for d in data:
        try:
    ...
    # Now read garcia_data.csv, 
    # which has columns: HMR, P_Loss_Coeff, FFR, CFR, BMR/HMR
    ...
    if os.path.exists(garcia_file):
        with open(garcia_file, 'r') as gf:
            ...
            for row in reader:
                ...
                # Fill only the columns you want, leave others blank
                garcia_dict = {
                    'Condition': 'Hyperemic',
                    ...
                    'CFR': cfr_g,
                    'BMR/HMR': bmr_hmr_g,
                    'CFR/FFR': cfr_ffr_g,       # can remain blank or compute if you want
                    'WSS_Area_Bifur': '',
                    'WSS_Bif': '',
                    'WSS_Avg_Area': '',
                    'WSS_LMB': '',
    ...
    # ----------------------------------------------------------------------------
    # 5) WRITE THE SUMMARY CSV FILE (with new "source" column)
    # ----------------------------------------------------------------------------
    header = [
        'Condition',
        'Geometry Number',
