...
summary_file = '/Users/tejjolly/Documents/BioSimm/Simulations/summary-HMR2.csv'
...
for name, group in grouped_cfr:
    if len(group) > 1:
        ...
        stenosis_formatted = f"{stenosis_val*100:2.0f}"
        length_formatted = f"{length_val:.1f}"
        ...
        # Add the line label
        label_text = f'S: {stenosis_formatted}%, L: {length_formatted}'
        if i == 3:
            plt.text(mean_ffr-.03, mean_cfr, label_text,
                     fontsize=10, color='gray', ha='center', va='center')
        else:
            plt.text(mean_ffr-.02, mean_cfr, label_text,
                     fontsize=10, color='gray', ha='center', va='center')
...
for name, group in grouped_third:
    ...
    if len(unique_r_values) > 1:
        ...
        if i == 2:
            print('entered')
            plt.text(mean_hmr, mean_ffr+.01, label_text,
                     fontsize=10, color='gray', ha='center', va='center')
        else:
            plt.text(mean_hmr, mean_ffr-.005, label_text,
                     fontsize=10, color='gray', ha='center', va='center')
        for idx, row in group.iterrows():
            if idx == 4:
                plt.text(row['HMR']+.4, row['P_d/P_a']+.002, f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')
            else:
                plt.text(row['HMR'], row['P_d/P_a']+.002, f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')
...
for name, group in grouped_fourth:
    ...
    if len(unique_r_values) > 1:
        ...
        mean_ffr = group['P_d/P_a'].mean()
        plt.text(mean_hsr+.08, mean_ffr, label_text,
                 fontsize=10, color='gray', ha='center', va='center')
        if i != 1:
            for idx, row in group.iterrows():
                plt.text(row['HSR'], row['P_d/P_a']-.02, f'R:{row["Rtotal_cor Value"]}',
                         fontsize=11, ha='right', va='bottom')
...
for name, group in grouped_hmr:
    ...
    if len(unique_r_values) > 1:
        ...
        mean_hmr = group['HMR'].mean()
        mean_hsr = group['HSR'].mean()
        plt.text(mean_hmr, mean_hsr+.025, label_text,
                 fontsize=10, color='gray', ha='center', va='center')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
            if idx == 4:
                plt.text(row['HMR']+.4, row['HSR'], f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')
            else:
                plt.text(row['HMR']+.15, row['HSR']+.02, f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')
...
# -----------------------------
# Plot: CFR vs HMR, Colored by HSR
# -----------------------------

# Filter out rows where 'CFR', 'HMR', or 'HSR' are NaN
...
boundaries_cfr_hmr = np.linspace(hsr_min_cfr_hmr, hsr_max_cfr_hmr, 6)  # 6 boundaries create 5 intervals
...
# Plot data points, colored by 'HSR'
scatter_cfr_hmr = plt.scatter(df_filtered_cfr_hmr['HMR'], df_filtered_cfr_hmr['CFR'],
                              c=df_filtered_cfr_hmr['HSR'], cmap=cmap_cfr_hmr, norm=norm_cfr_hmr,
...
for name, group in grouped_cfr_hmr:
    ...
    if len(unique_r_values) > 1:
        ...
        # mean_hmr = group['HMR'].mean()
        # mean_cfr = group['CFR'].mean()
        # plt.text(mean_hmr, mean_cfr*1.5, f'S:{stenosis_formatted}%',
        #          fontsize=10, color='gray', ha='center', va='center')
        # Plot the line label next to the first point
        first_point = group.iloc[0]  # Get the first row of the group
        plt.text(first_point['HMR']+.5, first_point['CFR'] - 0.2,  # Adjust positioning as needed
                 label_text, fontsize=10, color='gray', ha='center', va='bottom')
        # Add data point labels for Rtotal_cor Value
        for idx, row in group.iterrows():
            plt.text(row['HMR']+.3, row['CFR'] +.02, f'R:{row["Rtotal_cor Value"]}',
                     fontsize=11, ha='right', va='bottom')

# Add colorbar for the 'HSR' values
cbar_cfr_hmr = plt.colorbar(scatter_cfr_hmr, ticks=boundaries_cfr_hmr)
cbar_cfr_hmr.set_label('HSR [mmHg/cm/s]')
cbar_cfr_hmr.ax.yaxis.set_major_locator(plt.MaxNLocator(nbins=5))  # Ensure five ticks
