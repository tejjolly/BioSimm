...
# Plot the raw data (scatter)
plt.scatter(x_data, y_data, alpha=0.35, edgecolor='k', color='tab:blue', s=20, label="Data")

# Plot each fitted curve
plt.plot(x_fit, y_power, linestyle="--", color='red', label=f"Power-Law: y={power_params[0]:.2f}x^{power_params[1]:.2f}")
plt.plot(x_fit, y_exp, linestyle="-.", color='green', label=f"Exp: y={exp_params[0]:.2f}e^(-{exp_params[1]:.2f}x)")
plt.plot(x_fit, y_log, linestyle=":", color='orange', label=f"Log: y={log_params[0]:.2f}log(x) + {log_params[1]:.2f}")
...
for ax, model, residuals, predictions in zip(axes, models, residuals_list, predictions_list):
    # ax.scatter(predictions, residuals, alpha=0.7, color = 'tab:blue', edgecolor='tab:blue') # Uncomment for CFR/FFR on x-axis
    ax.scatter(x_data, residuals, alpha=0.7, color = 'tab:blue', edgecolor='tab:blue')
    ax.axhline(0, color='k', linestyle='--')
    # ax.set_xlabel(f"Predicted CFR/FFR ({model})") # Uncomment for CFR/FFR on x-axis
    ax.set_xlabel("HMR")
