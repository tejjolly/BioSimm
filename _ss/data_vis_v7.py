...
for name, group in grouped_cfr:
    if len(group) > 1:
        ...
        # Add the line label
        plt.annotate(f'S:{stenosis_formatted}%',
                     xy=(mean_ffr, mean_cfr),
                     xytext=(mean_ffr-.05, mean_cfr),
...
for name, group in grouped_hmr:
    ...
    if len(unique_r_values) > 1:
        ...
        plt.annotate(f'S:{stenosis_formatted}%',
             xy=(mean_hmr, mean_hsr),
             xytext=(mean_hmr, mean_hsr + .1),
             fontsize=10, color='gray',
             ha='center', va='center',
             arrowprops=dict(arrowstyle='-', color='gray', lw=0.5))
...
for name, group in grouped_third:
    ...
    if len(unique_r_values) > 1:
        ...
        mean_ffr = group['P_d/P_a'].mean()
        plt.annotate(f'S:{stenosis_formatted}%',
             xy=(mean_hmr, mean_ffr),  # The point to annotate (on the data line)
             xytext=(mean_hmr, mean_ffr -.05),  # Position of the label
             fontsize=10, color='gray',
             ha='center', va='center',
             arrowprops=dict(arrowstyle='-', color='gray', lw=0.5))
