...
#User inputs
user_epoch_input = 1000
user_batch_size = 8
user_hidden_dim = 3
user_lr = 1e-3
user_cos_restarts = 300
# ----------------------------------------------------------------
# 1) LOAD DATA
# ----------------------------------------------------------------
...
def train_one_fold(X_train_fold, y_train_fold, X_val_fold, y_val_fold,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=user_cos_restarts, T_mult=1, eta_min=1e-7)
...
# ----------------------------------------------------------------
# 4) CROSS VALIDATION + CORRELATION COLLECTION
# ----------------------------------------------------------------
n_splits = 5
...
corr_df = pd.DataFrame(fold_correlations, columns=feature_cols, index=[f"Fold {i+1}" for i in range(n_splits)])
corr_df.loc["Average"] = corr_df.mean(axis=0)
print("\nCorrelations of z with Input Features:")
