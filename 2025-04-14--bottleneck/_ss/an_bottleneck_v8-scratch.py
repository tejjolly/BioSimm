...
df_raw = df_raw[mask]
# df_raw['BMR/HMR/HMR'] *= df_raw['HMR']
# df_raw.rename(columns={"BMR/HMR/HMR": "BMR/HMR"}, inplace=True)
...
for i, col in enumerate(feature_cols):
    ...
    plt.axvline(mean - 2 * std, color='k', linestyle=":", linewidth=2, label="±1 SD" if i == 0 else "")
    plt.axvline(mean + 2 * std, color='k', linestyle=":", linewidth=2)
    plt.title(col)
    plt.xlabel("")
...
for col in feature_cols:
    mean = df_raw[col].mean()
    std  = df_raw[col].std()
    df_raw = df_raw.loc[(df_raw[col] >= mean - 2 * std) & (df_raw[col] <= mean + 2 * std)]
