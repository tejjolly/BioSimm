...
sns.heatmap(metrics_df, annot=True, fmt=".2f", cmap="Reds_r",
            linewidths=.5, linecolor="grey",vmin=.5, vmax=1)
plt.title(f"Average Per‑Class Metrics (5 Fold CV, training set, n={len(idx_train)})")
plt.ylabel("Metric"); plt.xlabel("Class"); plt.tight_layout();
savefig("metrics_cross-fold.png")
plt.show()
...
plt.figure(figsize=(8, 4))
sns.heatmap(metrics_test_df, annot=True, fmt=".2f", cmap="Reds_r",
            linewidths=.5, linecolor="grey",vmin=.5, vmax=1)
plt.title(f"Per-Class Metrics (test set, n={len(idx_test)})")
plt.ylabel("Metric"); plt.xlabel("Class")
plt.tight_layout();
savefig("metrics_test-set.png")
plt.show()
...
plt.tight_layout();
savefig("confusion_matrix_test-set.png")
plt.show()
...
if {"P_d/P_a", "CFR"}.issubset(df_raw.columns):
    ...
    plt.xlabel("FFR")
    plt.ylabel("CFR")
    plt.title(f"Test set predictions, n={len(idx_test)}")
    plt.grid(False)
    plt.tight_layout()
    savefig("cfr_ffr_preds.png")
    plt.show()
...
plt.tight_layout();
savefig("monte-carlo_median.png")
plt.show()
...
for val in sweep_vals:
    ...
    plt.tight_layout()
    savefig(f"monte-carlo_{sweep_feat}_{val:.2f}.png")
    plt.show()
