...
def savefig(name, **kw):                             # tiny helper
    # plt.savefig(os.path.join(save_dir, name), dpi=600, bbox_inches="tight", transparent=True, **kw)
    plt.savefig(os.path.join(save_dir, name) + ".png", dpi=800, bbox_inches="tight", transparent=True, **kw)
    plt.savefig(os.path.join(save_dir, name) + ".svg", bbox_inches="tight", transparent=True, **kw)
...
for i, col in enumerate(feature_cols):
    ...
    plt.subplot(1, 3, i + 1)
    sns.histplot(df_raw[col], kde=True, bins=25, color='#5E2F5C')
    # sns.histplot(df_raw[col], kde=True, bins=25, color='#5E9096')
    plt.axvline(median, color="#5E9096", linestyle="--", linewidth=1.2, label="Median")
    stdneg = mean - 1 * std
    stdpos = mean + 1 * std
    plt.axvline(stdneg, color='#F2AC7C', linestyle=":", linewidth=2, label="±1 SD" if i == 0 else "")
    plt.axvline(stdpos, color='#F2AC7C', linestyle=":", linewidth=2)
    plt.title(col)
    plt.xlabel("")
    plt.ylabel("Frequency" if i == 0 else "")
    plt.grid(False)
    print(f'{col}:')
    print(f'\n     stdnegdist:{mean-stdneg}, stdposdist: {stdpos-mean}, mean: {mean}')

plt.tight_layout()
savefig("sample_distribution")
plt.show()
...
# ax = sns.heatmap(metrics_df, annot=True, fmt=".2f", cmap="flare",
#             linewidths=.5, linecolor="grey", #vmin=.5,
#             vmax=1,
#             annot_kws={"fontsize": 16})
# # plt.title(f"Average Per‑Class Metrics (5 Fold CV, training set, n={len(idx_train)})")
# plt.ylabel("Metric", fontsize = 18); plt.xlabel("Class", fontsize = 18)
# ax.set_xticklabels(ax.get_xticklabels(), fontsize=2)
# ax.set_yticklabels(ax.get_yticklabels(), fontsize=16)
# plt.tight_layout()
# savefig("metrics_cross-fold")
# plt.show()
#
# ────────────────────────────────────────────────────────────────────────────────
...
# 8b)  HEAT-MAP OF PER-CLASS TEST-SET METRICS
cm_test = confusion_matrix(y_test, y_pred_test, labels=np.arange(num_classes))

prec_test = precision_score(y_test, y_pred_test,
                            average=None, zero_division=0)
sens_test = recall_score(   y_test, y_pred_test,
                            average=None, zero_division=0)
spec_test = specificity_per_class(cm_test)
...
ax = sns.heatmap(metrics_test_df, annot=True, fmt=".2f", cmap="flare",
            linewidths=1, linecolor="grey", #vmin=.5,
            vmax=1, annot_kws={"fontsize": 16})
# plt.title(f"Per-Class Metrics (test set, n={len(idx_test)})")
ax.set_xticklabels(ax.get_xticklabels(), fontsize=2)
ax.set_yticklabels(ax.get_yticklabels(), fontsize=16)
plt.ylabel("Metric", fontsize=18); plt.xlabel("Class",fontsize=18)
plt.tight_layout();
savefig("metrics_test-set")
plt.show()

# # 8c)  confusion matrix OF PER-CLASS TEST-SET METRICS
...
# plt.tight_layout();
# savefig("confusion_matrix_test-set")
# plt.show()
...
correct_circle = Line2D([0], [0], marker='o', color='grey', linestyle='None',
                        label='Correctly predicted',
                        markerfacecolor='lightgray', markersize=8, alpha=0.7)

wrong_circle = Line2D([0], [0], marker='^', color='k', linestyle='None',
...
if {"P_d/P_a", "CFR"}.issubset(df_raw.columns):
    ...
    sns.scatterplot(data=df_plot[~df_plot["mispred"]],
                    x="P_d/P_a", y="CFR",
                    hue="pred", palette="viridis",
                    marker="o", s=60, alpha=0.7, linewidth = 1.4, edgecolor='grey', legend=False)
    ...
    sns.scatterplot(data=df_plot[df_plot["mispred"]],
                    ...
                    hue="pred", palette="viridis",
                    marker="^", s=80, alpha=1,
                    edgecolor="k", linewidth=1.8, legend=False)
    ...
    plt.axhline(2.0, ls="--", c="gray", lw=3)
    plt.axvline(0.8, ls="--", c="gray", lw=3)
    plt.xlabel("FFR", fontsize = 20)
    plt.ylabel("CFR", fontsize = 20)
    plt.tick_params(axis='both', labelsize=18)
    # plt.title(f"Test set predictions, n={len(idx_test)}", fontsize=18)
    plt.grid(False)
    plt.tight_layout()
    savefig("cfr_ffr_preds")
    plt.show()
...
#     plt.xlabel(feat_x)
#     plt.ylabel(feat_y)
#     plt.title(f"Slice at {sweep_feat} = {val:.2f} +/- {eps}")
#     plt.tight_layout()
#     savefig(f"monte-carlo_{sweep_feat}_{val:.2f}")
#     plt.show()
...
sc = ax.scatter(mc_xyz[:, 0], mc_xyz[:, 1], mc_xyz[:, 2],
                c=mc_pred, cmap="viridis", alpha=1, s=3, linewidth=0)

# ax.set_xlabel(feat_x, fontsize = 20)
# ax.set_ylabel(r"$\zeta_{\mathrm{L}}$", fontsize=20)
# ax.set_zlabel(feat_z, fontsize = 20)
...
plt.tight_layout()
# savefig(f"monte-carlo_3-D")

dpi = 1200
plt.savefig(f"monte-carlo_3-D_{dpi}dpi",
            dpi=dpi,             # 600 dots/inch
            # bbox_inches="tight",
            format="png",
